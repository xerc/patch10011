 ...
 *
 * TOTAL FUNCTIONS: 32
 * (This index has been created by the extension "extdeveval")
 *
 */
...
final class ExtensionManagementUtility {
    ...
    */
    public static function addTCAcolumns($table, $columnArray, $addTofeInterface = 0) {
        ...
        if (is_array($columnArray) && is_array($TCA[$table]) && is_array($TCA[$table]['columns'])) {
            $TCA[$table]['columns'] = array_merge($TCA[$table]['columns'], $columnArray); // Candidate for GeneralUtility::array_merge() if integer-keys will some day make trouble...
            if ($addTofeInterface) {
    ...
    */
    public static function addModule($main, $sub = '', $position = '', $path = '') {
        ...
        if (isset($TBE_MODULES[$main]) && $sub) { // If there is already a main module by this name:
            ...
            $mods = GeneralUtility::trimExplode(',', $TBE_MODULES[$main], 1);
    ...
    * FOR USE IN ext_tables.php FILES
    * Example:  patch10011_extMgm::addModulePath('xMOD_tximpexp', patch10011_extMgm::extPath($_EXTKEY).'app/');
    *
    ...
    * FOR USE IN ext_tables.php FILES
    * eg. patch10011_extMgm::addLLrefForTCAdescr('pages', 'EXT:lang/locallang_csh_pages.xml'); for the pages table or patch10011_extMgm::addLLrefForTCAdescr('_MOD_web_layout', 'EXT:cms/locallang_csh_weblayout.php'); for the Web > Page module.
    * Usage: 31
    *
    ...
    */
    public static function addService($extKey, $serviceType, $serviceKey, $info) {
        ...
        if ($serviceType &&
                GeneralUtility::hasValidClassPrefix($serviceKey, array('user_')) &&
                is_array($info)) {
            ...
            if ($T3_SERVICES[$serviceType][$serviceKey]['available'] && $T3_SERVICES[$serviceType][$serviceKey]['os'] != '') {
                ...
                $os = GeneralUtility::trimExplode(',', strtoupper($T3_SERVICES[$serviceType][$serviceKey]['os']));
        ...
        if (is_array($T3_SERVICES[$serviceType])) {
            foreach ($T3_SERVICES[$serviceType] as $key => $info) {
                ...
                if ($info['available'] && ($info['subtype'] == $serviceSubType || $info['serviceSubTypes'][$serviceSubType]) && $info['priority'] >= $priority) {
                    ...
                    if (trim($info['exec'])) {
                        $executables = GeneralUtility::trimExplode(',', $info['exec'], 1);
    ...
    */
    public static function addToInsertRecords($table, $content_table = 'tt_content', $content_field = 'records') {
        global $TCA;
        GeneralUtility::loadTCA($content_table);
    ...
    public static function typo3_loadExtensions() {
        ...
        if ($rawExtList) {
            ...
            if ($extensionCacheBehaviour && self::isCacheFilesAvailable($cacheFilePrefix)) {
                ...
                $temp_extensions = array_unique(GeneralUtility::trimExplode(',', $rawExtList, 1));
...
?>';
        ...
        foreach ($extensions as $key => $conf) {
            if (is_array($conf)) {
                if ($conf['ext_localconf.php']) {
                    ...
                    $cFiles['ext_localconf'] .= trim(GeneralUtility::getUrl($conf['ext_localconf.php']));
                }
                if ($conf['ext_tables.php']) {
                    $cFiles['ext_tables'] .= self::_makeIncludeHeader($key, $conf['ext_tables.php']);
                    $cFiles['ext_tables'] .= trim(GeneralUtility::getUrl($conf['ext_tables.php']));
                }
            }
        }
        ...
        GeneralUtility::writeFile(PATH_typo3conf . $cacheFilePrefix . '_ext_localconf.php', $cFiles['ext_localconf']);
        GeneralUtility::writeFile(PATH_typo3conf . $cacheFilePrefix . '_ext_tables.php', $cFiles['ext_tables']);
    ...
    */
    public static function getCacheFilePrefix() {
        ...
        if ($extensionCacheBehaviour == 1) {
            $cacheFilePrefix .= '_ps' . substr(GeneralUtility::shortMD5(PATH_site . '|' . $GLOBALS['TYPO_VERSION']), 0, 4);
        } elseif ($extensionCacheBehaviour == 2) {
            $cacheFilePrefix .= '_' . GeneralUtility::shortMD5(self::getEnabledExtensionList());
        }
    ...
    */
    public static function getRequiredExtensionList() {
        $requiredExtensionList = GeneralUtility::uniqueList(
    ...
    */
    public static function getIgnoredExtensionList() {
        $ignoredExtensionList = GeneralUtility::uniqueList(
